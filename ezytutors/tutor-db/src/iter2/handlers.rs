...
// 핸들러가 몇 번 호출되었는지 추적하고, 애플리케이션 상태로 기록한 뒤 HTTP 응답에 포함하여 리턴
...
mod tests {
    ...
    async fn get_all_courses_success() {
        ...
        // 엔드-투-엔드 테스트에서는 Actix Web 프레임워크가 자동으로 애플리케이션 상태를 핸들러 함수에게 전달
        // (단위 테스트에서는 직접 수행해야 함)
        ...
        // 엔드-투-엔드 테스트에서는 Actix Web 프레임워크가 유입되는 HTTP 요청 매개변수를 역직렬화하여 핸들러 함수에 전달
        // (단위 테스트에서는 직접 수행해야 함)
